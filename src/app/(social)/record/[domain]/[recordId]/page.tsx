        ...
        >
          ...
          <div className="p-8 bg-slate-50/50">
             <div className="grid grid-cols-2 gap-y-6 gap-x-12">
               {Object.entries(mockRecord).map(([key, value]) => {
                 if (typeof value === 'object') return null; // skip subjects
                 return (
                   <div key={key}>
             ...
             {/* Nested Data (Subjects) */}
             <div className="mt-8 pt-6 border-t border-slate-200">
               <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-4">Subject Grades</div>
               <div className="space-y-3">
                 {Object.entries(mockRecord.subjects).map(([sub, data]) => (
                   <div key={sub} className="flex justify-between items-center bg-white p-3 rounded-lg border border-slate-200">
                     ...
                     <div className="flex gap-4">
                       <span className="text-xs font-bold text-slate-500">Credits: {data.credits}</span>
                       <span className="text-xs font-black text-primary px-2 py-0.5 bg-primary/10 rounded">{data.grade}</span>
                     </div>
                   </div>
                 ))}
               </div>
